...
### 2.1 Standard Finsler-Type Structure
...
- **MEM01:** memory-channel ablation test showing that the memory channel is not a Shapiro-time contribution and has its strongest current role in EL/Fermat bridge behavior.
- **TRM84–TRM87:** memory-scaling guards: quadratic \(\phi\)-dependence, linear \(|\dot{\mu}|\)-dependence, weak-field subleading behavior, and separation from pure time-channel behavior.
- **MC01–MC08:** completed local invariant-selection block for the memory channel.
- **HOA01:** verifies that the implemented transport index depends on position, direction state, and directional-change rate.

These tests validate bounded behavior and bridge consistency. TRM84–TRM87 plus MC01–MC08 establish the local memory-channel selection/consistency block, and HOA01 supports the higher-order optical-action interpretation. They do **not** by themselves close the full variational first-principles derivation.
