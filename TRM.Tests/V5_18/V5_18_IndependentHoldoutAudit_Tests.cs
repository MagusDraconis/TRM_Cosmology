...
[Trait("Category","V5_18"),Trait("Category","V5_18_AGI"),Trait("Category","LongRunning")]
public class V5_18_IndependentHoldoutAudit_Tests
{
    ...
    public V5_18_IndependentHoldoutAudit_Tests(ITestOutputHelper o){_o=o;}
    static ConcurrentDictionary<int,P3>? _hC,_lC;
    P3 Hi(int n){_hC??=new();return _hC.GetOrAdd(n,k=>PCent(k,true));}
    P3 Lo(int n){_lC??=new();return _lC.GetOrAdd(n,k=>PCent(k,false));}

    [Fact]public void AGI_01_IndependentHoldoutAudit(){
        _o.WriteLine("═══ AGI_01: Independent holdout audit (300-399) ═══");
        _o.WriteLine("Hostile validation: testing M3++ on unseen cohort 300-399.");

        var results=new ConcurrentBag<(int n,int s,int coh,bool a0,bool c3)>();

        // Test cohort 300-399 at N=71/72 (10 seeds each)
        foreach(var n in new[]{71,72}){
            var hi=Hi(n);var lo=Lo(n);int cnt=0;
            for(int s=300;s<=399&&cnt<10;s++){
                ...
                results.Add((n,s,3,a0,c3p));
            }
        }

        var all=results.ToArray();
        _o.WriteLine($"Independent holdout seeds: {all.Length}");

        _o.WriteLine(string.Format("\n{0,4} {1,6} {2,6} {3,6} {4,6} {5,6} {6,8}",
            "N","Seeds","A0","C3","Resc","Dam","Lift"));
        foreach(var n in new[]{71,72}){
            ...
            _o.WriteLine($"{n,4} {sub.Length,6} {a0s*100.0/sub.Length,5:F0}% {c3s*100.0/sub.Length,5:F0}% {resc,6} {dam,6} {(c3s-a0s)*100.0/sub.Length,7:+.0}%");
        }

        // Combined with prior cohort data (from AGE: Ho1=100-199, Ho2=200-299)
        // Simplified: just report current findings
        int totalA0=all.Count(r=>r.a0),totalC3=all.Count(r=>r.c3);
        int totalResc=all.Count(r=>!r.a0&&r.c3),totalDam=all.Count(r=>r.a0&&!r.c3);

        _o.WriteLine($"\n─── Independent holdout summary ───");
        _o.WriteLine($"A0: {totalA0}/{all.Length} ({totalA0*100.0/all.Length:F0}%)");
        _o.WriteLine($"C3: {totalC3}/{all.Length} ({totalC3*100.0/all.Length:F0}%)");
        _o.WriteLine($"Rescues: {totalResc}, Damage: {totalDam}");
        _o.WriteLine($"Lift: +{(totalC3-totalA0)*100.0/all.Length:.0}pp");

        // Gates
        bool survives=totalC3>totalA0&&totalDam==0;
        bool liftPositive=(totalC3-totalA0)*100.0/all.Length>0;
        bool noNewDamage=totalDam==0;
        bool cohort3OK=survives;

        _o.WriteLine($"\n─── Hostile audit gates ───");
        _o.WriteLine($"Gate A (Independent confirms): {(survives?$"SURVIVED — +{(totalC3-totalA0)*100.0/all.Length:.0}pp":"FAILED")}");
        _o.WriteLine($"Gate B (Lift positive): {(liftPositive?"SURVIVED":"FAILED")}");
        _o.WriteLine($"Gate C (No new damage): {(noNewDamage?"SURVIVED":"FAILED")} — {totalDam} damage");
        _o.WriteLine($"Gate D (Boundary unchanged): CHECK AGE data");
        _o.WriteLine($"Gate E (Overfit evidence): {(survives?"NOT FOUND — no collapse":"FOUND")}");
        _o.WriteLine($"Gate F (Survives audit): {(survives?"SURVIVED — M3++ robust":"FAILED")}");

        _o.WriteLine($"\n─── Conclusion ───");
        _o.WriteLine(survives?"M3++ survives independent holdout audit. No evidence of overfit.":"M3++ fails independent holdout. Investigate collapse.");
        _o.WriteLine("\n─── Next: AGS Final Synthesis ───");
    }
