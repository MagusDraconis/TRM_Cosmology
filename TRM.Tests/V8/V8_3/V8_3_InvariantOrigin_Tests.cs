...
public class V8_3_InvariantOrigin_Tests
{
    ...
    [Fact]
    public void TTA_01_TickGeneratorAudit()
    {
        _o.WriteLine(new string('=', 108));
        _o.WriteLine("=== TTA_01: Tick Generator Audit ===");
        _o.WriteLine("=== Is there a universal primitive tick? ===");
        _o.WriteLine(new string('=', 108));

        const int baseSeed = 24319;
        const double xiBase = 2.95;
        const double k0Base = 1.0;
        ...
        // Collect per-β: β, entropy, dH/dβ, λ1, accessibility, dimension
        var points = new List<(VcFamily fam, double beta, double ent, double dH, double lam1, double acc, double dimVal)>();
        ...
        foreach (var fam in families)
        {
            ...
            {
                var cfg = configs[ci];
                var pts = new List<(double beta, double ent, double lam1, double dimVal)>();

                for (int bi = 0; bi < nBeta; bi++)
                {
                    double beta = bi / (double)(nBeta - 1);
                    var v = new VariantSpec($"{fam}_TT", fam, cfg.alpha, 1.0, cfg.xiScale, beta, 0.0);
                    var allC = new List<double[]>(); var allL = new List<double>();
                    double xi = xiBase * v.XiScale, k0 = k0Base * v.K0Scale;
                    ...
                    pts.Add((beta, ent, sEE[0], dimVal));
                }

                for (int i = 0; i < pts.Count - 1; i++)
                {
                    double dH = Math.Abs(pts[i + 1].ent - pts[i].ent) / (1.0 / (nBeta - 1));
                    double d1a = Math.Abs((pts[i].dimVal > 0 ? Math.Log(pts[i].dimVal) : 1.0) - 1.0);
                    double d2a = Math.Abs((pts[i].dimVal > 0 ? Math.Log(pts[i].dimVal) : 1.0) - 0.5);
                    double d3a = Math.Abs((pts[i].dimVal > 0 ? Math.Log(pts[i].dimVal) : 1.0) - 1.0 / 3.0);
                    double acc = 1.0 / Math.Max(Math.Min(d1a, Math.Min(d2a, d3a)), 0.01);
                    points.Add((fam, pts[i].beta, pts[i].ent, dH, pts[i].lam1, acc, pts[i].dimVal));
                }
            }
        }

        // ============================================================
        // Tick candidates: which quantities are universal?
        // ============================================================
        _o.WriteLine("=== Tick Candidate Universality ===");
        _o.WriteLine($"{"Family",-6} {"ρ(β,ent)",10} {"ρ(β,dim)",10} {"CV(dH)",10} {"dH>0",8} {"mean dH",10}");
        _o.WriteLine(new string('-', 56));

        int alwaysPositiveDH = 0;
        foreach (var fam in families)
        {
            var fd = points.Where(p => p.fam == fam).ToArray();
            double rhoBE = SpearmanRank(fd.Select(p => p.beta).ToArray(), fd.Select(p => p.ent).ToArray());
            double rhoBD = SpearmanRank(fd.Select(p => p.beta).ToArray(), fd.Select(p => p.dimVal).ToArray());
            double cvd = StdOverMean(fd.Select(p => p.dH).ToArray());
            bool allPositive = fd.All(p => p.dH > 1e-12);
            if (allPositive) alwaysPositiveDH++;
            double meanDH = fd.Average(p => p.dH);
            string posLabel = allPositive ? "YES" : "no";
            _o.WriteLine($"{fam,-6} {rhoBE,10:F4} {rhoBD,10:F4} {cvd,10:F4} {posLabel,8} {meanDH,10:F4}");
        }
        _o.WriteLine("");

        bool entMonotonic = true;
        foreach (var fam in families)
        {
            var fd = points.Where(p => p.fam == fam).ToArray();
            double rho = SpearmanRank(fd.Select(p => p.beta).ToArray(), fd.Select(p => p.ent).ToArray());
            if (rho < 0.95) entMonotonic = false;
        }
        _o.WriteLine($"Entropy monotonic: {(entMonotonic ? "YES — universal tick" : "NO — not universal")}");
        _o.WriteLine($"dH always positive: {alwaysPositiveDH}/{families.Length}");
        _o.WriteLine("");

        // ============================================================
        // Is entropy a consequence of ticking or the source?
        // ============================================================
        _o.WriteLine("=== Tick Source ===");
        // τ = β (the parameter) — universal, monotonic, exogenous
        // Entropy always increases with τ → entropy IS the tick count
        // dH/dβ > 0 means ticking is always forward

        var allEnt = points.Select(p => p.ent).ToArray();
        var allBeta = points.Select(p => p.beta).ToArray();
        double rhoGlobal = SpearmanRank(allBeta, allEnt);

        _o.WriteLine($"Global ρ(β, entropy) = {rhoGlobal:F4}");
        _o.WriteLine($"N = {points.Count} points");
        _o.WriteLine("");
        ...
        bool universalTick = rhoGlobal > 0.95 && alwaysPositiveDH == families.Length;
        bool familyTick = rhoGlobal > 0.7;

        string decision;
        if (universalTick) decision = "Model C";
        else if (familyTick) decision = "Model B";
        else decision = "Model A";

        _o.WriteLine($"Decision: {decision}  ρ(β,ent)={rhoGlobal:F4}  dH>0: {alwaysPositiveDH}/{families.Length}");

        if (decision == "Model C")
            _o.WriteLine("β IS the universal tick generator. Entropy increases monotonically with β across all families, dH/dβ is always positive, and ordering emerges from tick accumulation. Time is β-counting — no deeper mechanism needed.");
        else if (decision == "Model B")
            _o.WriteLine("Family-dependent ticks.");
        else
            _o.WriteLine("No universal tick.");

        _o.WriteLine("");
        _o.WriteLine("=== TTA_01 complete. Commit: TTA_01_TickGeneratorAudit ===");
        Assert.True(new[] { "Model A", "Model B", "Model C", "Model D" }.Contains(decision));
    }
    ...
    {
        int n = x.Length;
        var xi = x.Select((v, i) => (v, i)).OrderBy(p => p.v).Select((p, r) => (p.i, rank: r + 1.0)).OrderBy(p => p.i).Select(p => p.rank).ToArray();
        var yi = y.Select((v, i) => (v, i)).OrderBy(p => p.v).Select((p, r) => (p.i, rank: r + 1.0)).OrderBy(p => p.i).Select(p => p.rank).ToArray();
        return PearsonCorrelation(xi, yi);
    }
